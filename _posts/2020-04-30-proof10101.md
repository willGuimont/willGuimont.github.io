...
Let's take a small break to discuss the divisibility by 11. For a number $$n$$ to be divisible by $$11$$, we must have that $$n \equiv 0 \pmod {11}$$
...
For $$n$$ to be even, we must have that $$k$$ is odd, because $$k=n-1$$.

For $$k\geq3$$, We have that $$\overbrace{11\ldots11}^{n\text{-times}} > 11$$, because $$n=k+1\geq4$$.


We can deduce that $$\frac{\overbrace{11\ldots11}^{n\text{-times}}}{11} > 1$$. Posing $$a=\frac{\overbrace{11\ldots11}^{n\text{-times}}}{11}$$, we get $$A(k) = (10^n + 1)a$$. Since $$A(k)$$ is the result of the multiplication between two integers (not equal to one), $$A(k)$$ is not prime.

We can use a similar trick for any even $$k\geq2$$, this time with the other part of $$A(k)$$. When $$k$$ is even (so that $$n$$ is odd) in $$(10^n + 1)$$, we have that one $$1$$ is at a even position and the other at an odd position, thus $$1 - 1 = 0$$ which is divisible by $$11$$. Similarly, we have that $$(10^n + 1)>11$$ because $$n\geq3$$. Thus, $$\frac{(10^n + 1)}{11}>1$$. We can then conclude that $$A(k)$$ is not prime for even $$k\geq2$$.

This leaves us with two cases, $$k=0$$ and $$k=1$$. $$A(0) = 1$$ is not prime by definition and $$A(1)$$ is prime. Thus $$A(1)$$ is the only prime in the sequence.
